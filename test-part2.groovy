...
class Dev extends ScmExecutable {
	...
	def gatekeeper_update_svn_from_bare(branch) {
		...
		def repo = new GitRepo( repoDir : wdir + "/devs/adm/" + branch + ".git.gatekeeper" )
		repo.git( "fetch", "bare_repo")
		repo.git( "rebase", "remotes/bare_repo/" + branch)
		repo.git( "svn", "rebase" )
...
println( "--")
per.git().checkout( "trunk" ).chdir( "web" ).touch_and_add( "readme-from-per.txt" ).commit().push();
...
println( "--")
ola.git().checkout( "kaksi" ).chdir( "web" ).touch_and_add( "readme-from-ola.txt" ).commit().push();
...
println( "--")
siv.git().checkout( "yksi" ).chdir( "web" ).touch_and_add( "readme-from-siv.txt" ).commit().push();
...
siv.git().checkout( "trunk" ).chdir( "web" ).touch_and_add( "siv-next-solution.txt" ).commit().push();
per.git().checkout( "trunk" ).chdir( "web" ).remove( "siv-next-solution.txt" ).commit().push();
adm.gatekeeper_update_svn_from_bare("trunk").svn( "up" )
...
siv.git().checkout( "trunk" ).chdir( "web" ).touch_and_add( "siv-x2-solution.txt" ).commit().push();
siv.git().on_file( "siv-x2-solution.txt" ).append( "SivSuggestion" ).commit().push();
per.git().checkout( "trunk" ).chdir( "web" ).on_file("siv-x2-solution.txt").append( "PerSuggestion" ).commit().push();
adm.gatekeeper_update_svn_from_bare("trunk").svn( "up" )
...
siv.git().checkout( "trunk" ).git( "pull", "--rebase" )
siv.checkout( "yksi" ).chdir( "web" ).on_file( "readme.txt" ).append( "Siv->Web2.0Rocks-YouKnow" ).commit().push();

ola.git().checkout( "yksi" ).git( "pull", "--rebase" )
ola.git().checkout( "kaksi" ).git( "cherry-pick", "remotes/yksi_bare/yksi~0");
ola.chdir( "web" ).on_file( "readme.txt" ).append( "Ola->JavaScriptRocksYouKnow" ).commit().push();

per.git().checkout( "yksi" ).git( "pull", "--rebase" )
per.git().checkout( "kaksi" ).git( "pull", "--rebase" )
per.git().checkout( "trunk" ).git( "cherry-pick", "remotes/yksi_bare/yksi~0").git( "cherry-pick", "remotes/kaksi_bare/kaksi~0");
